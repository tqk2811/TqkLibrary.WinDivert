...
public sealed class TcpRelayServer : IDisposable
{
    ...
    public TcpRelayServer(NatTable nat, TcpConnectionHandler? handler, RedirectLogger? logger = null, bool enableIpv6 = false)
    {
        _nat = nat;
        _handler = handler;
    ...
    public void Start()
    {
        ...
        if (_listenerV6 != null)
        {
            ...
            try
            {
                ...
                _log.Log("RLY", $"TcpRelay listening on [::1]:{PortV6}");
                _acceptLoops.Add(Task.Run(() => AcceptLoop(_listenerV6, isIpv6: true, _cts.Token)));
            }
            catch (SocketException ex)
            {
                PortV6 = 0;
                _log.Log("RLY", $"TcpRelay could not listen on [::1]: {ex.SocketErrorCode}");
            }
        }
    }
    ...
    private async Task AcceptLoop(TcpListener listener, bool isIpv6, CancellationToken ct)
    {
        ...
        while (!ct.IsCancellationRequested)
        {
            ...
            try
            {
                ...
                _log.Log("RLY", $"Accepted[{tag}] from {rep}");
            }
            catch (ObjectDisposedException) { _log.Log("RLY", $"AcceptLoop[{tag}] disposed"); return; }
            catch (SocketException ex) { _log.Log("RLY", $"AcceptLoop[{tag}] SocketException: {ex.SocketErrorCode}"); return; }
    ...
    private async Task HandleAsync(TcpClient client, bool isIpv6, CancellationToken ct)
    {
        ...
        if (entry == null)
        {
            _log.Log("RLY", $"Handle: NAT miss for srcPort={remote.Port} ipv6={isIpv6}, closing");
            client.Close();
            return;
        }
        _log.Log("RLY", $"Handle: matched srcPort={remote.Port} ipv6={isIpv6} -> origDst={entry.OriginalDestinationAddress}:{entry.OriginalDestinationPort}");
