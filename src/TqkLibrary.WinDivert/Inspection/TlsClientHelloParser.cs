...
public static class TlsClientHelloParser
{
    ...
    public const int RecommendedPeekSize = 2048;

    // True when the buffer starts like a TLS handshake record — cheap pre-check before peeking
    // further or attempting a full parse.
    public static bool LooksLikeTls(byte[] buffer, int length)
        => buffer != null && length >= 3 && buffer[0] == HandshakeContentType && buffer[1] == 0x03;

    public static bool TryReadServerName(byte[] buffer, int length, out string serverName)
    {
        serverName = string.Empty;
        if (!LooksLikeTls(buffer, length)) return false;
