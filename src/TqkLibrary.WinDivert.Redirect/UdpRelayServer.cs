...
// destination.
public sealed class UdpRelayServer : IUdpRelayServer
{
    ...
    public UdpRelayServer(INatTable nat, UdpDatagramHandler? handler, bool enableIpv6 = false)
    {
    ...
    // Packed into an int rather than a tuple purely because an int key allocates nothing on lookup.
    ...
    // The packet is emitted FROM the relay listener (so its source is the relay port the NAT
    // stage recognises on its reply leg), TO the loopback address of the right family at
    // processClientPort. The NAT stage then rewrites it back to origDst -> origSrc and reinjects
    // it on the real interface.
